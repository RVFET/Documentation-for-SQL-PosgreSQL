...
SUM(CommissionSum) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,
...
SUM(PaySum) Amount,SUM(ProviderSum) ProviderAmount,SUM(CommissionSum) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,
Agent,p.ProviderName 
...
select ID,ServiceName, SUM(PaySum) Amount,SUM(ProviderSum) ProviderAmount, SUM(CommissionSum) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,
...
UNION ALL --- Alkredit
select ID,ServiceName, SUM(PaySum) Amount,SUM(ProviderSum) ProviderAmount, 0.236*count(*) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,
Agent,p.ProviderName from (select Cast(OsmpProviderID as nvarchar(30)) ID,pp.ProviderName,ServiceName,
	  PaySum,ProviderSum,CommissionSum
	  , CAST([ExtraParams].query('data(r/agt_id)') as nvarchar) as Agent
  FROM [dbo].[Payment] p with (nolock)
...
select ID,ServiceName, SUM(PaySum) Amount,SUM(ProviderSum) -SUM(CommissionSum) ProviderAmount, SUM(CommissionSum) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,
Agent,p.ProviderName from (select Cast(OsmpProviderID as nvarchar(30)) ID,pp.ProviderName,ServiceName,
...
select ID,ServiceName, SUM(PaySum) Amount,SUM(ProviderSum) ProviderAmount, SUM(CommissionSum) CommissionAmount, 0.00 CardCommissionAmount, count(*) Count,Agent,p.ProviderName from (
...
group by ID, ServiceName, Agent,p.ProviderName
order by p.ProviderName,ServiceName
