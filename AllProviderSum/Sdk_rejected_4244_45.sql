--Get Report Rejected Payments - Failed
select "P_ID","ID","Service Name", "Amount", "Description", "Agent", "ChargeCommission", "PurchaseCommission", "OtherCommissions", "status"  from (
	...
	with commissions as 
		(			
					...
					where ha.performedat >='2021-05-01' and ha.performedat <'2021-06-01'
		)
		...
		select gtd.status "status", gtd.external_id "P_ID",pp.external_id::text  "ID",pp.display_name "Service Name",pp.description "Description",tbb.id "Process ID", gtd.amount_to_send "Amount", t."ChargeCommission",t."PurchaseCommission",t."OtherCommissions",gtd.amount_to_send+t."ChargeCommission"+t."PurchaseCommission" "Total",'17' "Agent" from commissions t 
		...
		union 
		select gtd.status "status",gtd.external_id "P_ID",pp.external_id::text  "ID",pp.display_name "Service Name",pp.description "Description",tbb.id "Process ID", gtd.amount_to_send "Amount", t."ChargeCommission",t."PurchaseCommission",t."OtherCommissions",gtd.amount_to_send+t."ChargeCommission"+t."PurchaseCommission" "Total",'18' "Agent" from commissions t 
